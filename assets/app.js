...
function renderPubs(){const q=$("#pubSearch").value.toLowerCase(),type=$("#pubType").value,year=ys.value;
 const list=publications.filter(p=>(type==="all"||p[1]===type)&&(year==="all"||p[0]===year)&&p.join(" ").toLowerCase().includes(q));
 $("#pubList").innerHTML=list.map(p=>`<article class="pub"><div class="tag">${p[1]} · ${p[0]}</div><h3>${p[2]}</h3><p>${p[3]}</p></article>`).join("")||`<div class="card">No matching publication found.</div>`;
}
